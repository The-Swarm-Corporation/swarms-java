...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { SwarmsClientInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw SwarmsClientInvalidDataException("`$name` is not set")
            is JsonNull -> throw SwarmsClientInvalidDataException("`$name` is null")
            else -> throw SwarmsClientInvalidDataException("`$name` is invalid, received $this")
        }
