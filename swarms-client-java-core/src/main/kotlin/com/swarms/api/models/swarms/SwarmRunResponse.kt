...
private constructor(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is SwarmRunResponse && id == other.id && description == other.description && executionTime == other.executionTime && numberOfAgents == other.numberOfAgents && output == other.output && serviceTier == other.serviceTier && status == other.status && swarmName == other.swarmName && swarmType == other.swarmType && usage == other.usage && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(id, description, executionTime, numberOfAgents, output, serviceTier, status, swarmName, swarmType, usage, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "SwarmRunResponse{id=$id, description=$description, executionTime=$executionTime, numberOfAgents=$numberOfAgents, output=$output, serviceTier=$serviceTier, status=$status, swarmName=$swarmName, swarmType=$swarmType, usage=$usage, additionalProperties=$additionalProperties}"
}
